...
| [AnimatedIconService.ts](file:///r:/Obsidian/Testsub1/.obsidian/plugins/colorful-folders/src/services/AnimatedIconService.ts) | Coordinates live SMIL animated icon injection. Isolates animated SVGs (`<animate>`, `<animateTransform>`, `<set>`) into `.cf-live-animated-icon` containers with zero performance overhead and clean lifecycle garbage collection. | `isAnimatedIcon()`, `getAnimatedIconElement()`, `syncAnimatedIcons()`, `invalidateCache()` |
