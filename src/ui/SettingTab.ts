...
export class ColorfulFoldersSettingTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        const packs = [
            { name: "✨ Obsidian Official (Lucide)", desc: "Native built-in Obsidian icon pack (1400+ Lucide icons).", url: "https://lucide.dev", prefix: "lucide", isBuiltIn: true },
        ...
        packs.forEach(p => {
            ...
            if (p.isBuiltIn) {
                const activeBadge = btnGroup.createEl("span", { text: "✓ Built-in Active" });
                activeBadge.setCssStyles({
                    padding: "6px 12px",
                    borderRadius: "6px",
        ...
        // ──────────────────────────────────────────────────────────────────────
        // ── PRIVACY & SYSTEM MAINTENANCE PANEL ────────────────────────────────
        // ──────────────────────────────────────────────────────────────────────
        ...
        const privacyCard = makeCard(sysPanel, "🔒", "Privacy & Data Protection");
        const privInfo = privacyCard.createDiv();
        privInfo.setCssStyles({
            padding: "12px 16px",
        ...
        const privTitle = privInfo.createDiv({ text: "✓ 100% Offline & Private" });
        privTitle.setCssStyles({ fontWeight: "700", color: "var(--text-success)", marginBottom: "4px" });
        const privDesc = privInfo.createDiv({ text: "Colorful Folders operates completely locally inside your Obsidian vault. No user data, folder names, or telemetry are ever collected or sent over the network." });
        privDesc.setCssStyles({ fontSize: "0.85em", color: "var(--text-muted)", lineHeight: "1.4" });

        const backupCard = makeCard(sysPanel, "💾", "Settings backup & management");
        new obsidian.Setting(backupCard)
            .setName("Export settings JSON")
            .setDesc("Copy your current Colorful Folders configuration as a JSON string to backup or transfer to another vault.")
            .addButton(btn => btn
                .setButtonText("Copy Settings JSON")
                .onClick(() => {
                    const json = JSON.stringify(this.plugin.settings, null, 2);
                    navigator.clipboard.writeText(json);
                    new obsidian.Notice("Settings JSON copied to clipboard!");
                }));

        let importJsonVal = "";
        new obsidian.Setting(backupCard)
            .setName("Import settings JSON")
            .setDesc("Paste an exported settings JSON configuration to restore your settings.")
            .addTextArea(text => {
                text.setPlaceholder("Paste settings JSON here...")
                    .onChange(val => { importJsonVal = val; });
                text.inputEl.setCssStyles({ width: "100%", height: "80px", fontFamily: "var(--font-monospace)", fontSize: "0.85em" });
            })
            .addButton(btn => btn
                .setButtonText("Restore Settings")
                .setCta()
                .onClick(async () => {
                    if (!importJsonVal.trim()) return;
                    try {
                        const parsed = JSON.parse(importJsonVal.trim());
                        this.plugin.settings = Object.assign({}, DEFAULT_SETTINGS, parsed);
                        await this.plugin.saveSettings();
                        RainbowManager.clearCache();
                        this.plugin.generateStylesDebounced();
                        new obsidian.Notice("Settings successfully restored!");
                        (this as unknown as { display: () => void }).display();
                    } catch (e) {
                        new obsidian.Notice("Invalid JSON configuration format.", 5000);
                    }
                }));

        new obsidian.Setting(backupCard)
            .setName("Reset all settings to default")
            .setDesc("Reset all plugin configurations back to default settings.")
            .addButton(btn => {
                btn.setButtonText("Reset to Defaults");
                (btn as unknown as { setWarning: () => typeof btn }).setWarning();
                btn.onClick(() => {
                    new ConfirmModal(this.app, "Reset All Settings", "Are you sure you want to reset all Colorful Folders settings back to defaults?", async () => {
                        this.plugin.settings = Object.assign({}, DEFAULT_SETTINGS);
                        await this.plugin.saveSettings();
                        RainbowManager.clearCache();
                        this.plugin.generateStylesDebounced();
                        new obsidian.Notice("All settings reset to default.");
                        (this as unknown as { display: () => void }).display();
                    }).open();
                });
            });
        ...
        if (this.plugin.settings.rainbowRootText) {
            ...
            const updateRainbowPreview = () => {
                ...
                const palette = this.plugin.getActivePalette(isDark);
