...
/**
 * DividerManager ΓÇö Complete rewrite for stability.
 */
export class DividerManager {
    ...
    public buildDividerNode(path: string, conf: FolderStyle, doc: Document): HTMLElement {
        ...
        const folderStyle = this.plugin.settings.customFolderColors?.[path];
        const effectiveStyle = typeof folderStyle === 'object' && folderStyle ? folderStyle : {};
        const rawIcon = (conf.dividerIcon !== undefined && conf.dividerIcon !== '')
            ? conf.dividerIcon.trim()
            : (conf.icon ? conf.icon.trim() : (effectiveStyle.icon ? effectiveStyle.icon.trim() : ''));
        const configKey = `${name}_${color}_${isUpper}_${alignment}_${useGlass}_${lineStyle}_${pillMode}_${rawIcon}_${conf.dividerIconColor || ''}_${dividerThickness}_${padLeft}_${padRight}_${conf.dividerDescription || ''}_${iconPosition}`;
        div.dataset.dividerKey = configKey;
        ...
        // ΓöÇΓöÇ Chip (pill label) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        // Icon (Lucide, custom SVG, or emoji)
        const addIcon = () => {
            if (!rawIcon) return;
            ...
            if (isCustom) {
                ...
                // Try rendering via obsidian.setIcon first
                ...
                if (svg) {
                    ...
                    // Fallback to emoji or unknown text icon
                    iconWrap.remove();
        ...
        // ΓöÇΓöÇ Event handlers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        div.oncontextmenu = (e) => {
            e.preventDefault();
    ...
    private syncContainer(container: Element) {
        ...
        try {
            ...
            // ΓöÇΓöÇ Step 2: Index existing divider nodes by target path ΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            for (const [path, { conf, isGlobal }] of desired) {
                ...
                const name = conf.dividerText || (path === 'cf-global-files-separator' ? (this.plugin.settings.fileDividerText || 'Files') : 'Section');
                const color = conf.dividerColor || (path === 'cf-global-files-separator' ? (this.plugin.settings.separatorColor || 'var(--text-muted)') : 'var(--interactive-accent)');
                const isUpper = conf.dividerUpper !== false;
                const alignment = conf.dividerAlignment || 'center';
                const useGlass = conf.dividerGlass !== undefined ? conf.dividerGlass : this.plugin.settings.glassmorphism;
                const lineStyle = conf.dividerLineStyle && conf.dividerLineStyle !== 'global' ? conf.dividerLineStyle : (this.plugin.settings.dividerLineStyle || 'solid');
                const iconPosition = conf.dividerIconPosition || 'left';
                const pillMode = conf.dividerPillMode ? (conf.dividerPillMode === 'on') : (this.plugin.settings.dividerPillMode !== false);
                const padLeft = conf.dividerLinePaddingLeft ?? this.plugin.settings.dividerLinePaddingLeft ?? 8;
                const padRight = conf.dividerLinePaddingRight ?? this.plugin.settings.dividerLinePaddingRight ?? 8;
                const dividerThickness = conf.dividerThickness || this.plugin.settings.dividerThickness || 1.5;
                const folderStyleSync = this.plugin.settings.customFolderColors?.[path];
                const effectiveStyleSync = typeof folderStyleSync === 'object' && folderStyleSync ? folderStyleSync : {};
                const rawIconSync = (conf.dividerIcon !== undefined && conf.dividerIcon !== '')
                    ? conf.dividerIcon.trim()
                    : (conf.icon ? conf.icon.trim() : (effectiveStyleSync.icon ? effectiveStyleSync.icon.trim() : ''));
                const configKey = `${name}_${color}_${isUpper}_${alignment}_${useGlass}_${lineStyle}_${pillMode}_${rawIconSync}_${conf.dividerIconColor || ''}_${dividerThickness}_${padLeft}_${padRight}_${conf.dividerDescription || ''}_${iconPosition}`;

                if (existing) {
                    if (existing.dataset.dividerKey === configKey && existing.parentElement === targetEl) {
                        targetEl.classList.add('cf-has-divider');
                        continue; // ZERO DOM touch if node already matches configKey!
                    }
